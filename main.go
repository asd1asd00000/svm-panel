...
// iranTime is centralized in the models package so it is defined only once.
var iranTime = models.IranTime
...
func main() {
	...
	for {
		...
		case 2:
			...
			apiPort := os.Getenv("SVM_API_PORT")
			if apiPort == "" {
				apiPort = "8080"
			}
			resp, err := client.Get("http://127.0.0.1:" + apiPort + "/api/internal/online")
			if err == nil {
		...
		case 3:
			...
			case 3:
				...
				if token != "" {
					_ = database.SetSetting("tg_bot_token", token)
				}
				fmt.Print("Telegram Chat ID: ")
				_, _ = fmt.Scanln(&chatID)
				if chatID != "" {
					_ = database.SetSetting("tg_chat_id", chatID)
				}
				fmt.Print("Auto-Backup Zip Password: ")
				_, _ = fmt.Scanln(&pass)
				if pass != "" {
					_ = database.SetSetting("zip_password", pass)
				}
				fmt.Print("Auto-Backup Interval (Hours): ")
				_, _ = fmt.Scanln(&hours)
				if hours != "" {
					_ = database.SetSetting("auto_backup_hours", hours)
				}
			case 4:
				reader := bufio.NewReader(os.Stdin)
				...
				if annStr != "" {
					_ = database.SetSetting("announcement_url", annStr)
				}
				...
				if tutStr != "" {
					_ = database.SetSetting("tutorial_url", tutStr)
				}
		...
		case 4:
			...
			if nodeMenuChoice == 1 {
				...
				if remarkStr == "" {
					remarkStr = targetNode.CustomRemark
				}
